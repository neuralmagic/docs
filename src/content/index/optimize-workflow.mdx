...
metaDescription: "Overview of the capabilities for optimizing a model for inference in the DeepSparse platform"
index: 1000
---

# Optimize a Model for Inference

The first workflow supported by the DeepSparse Platform is optimizing a model for inference.

### Motivation 

There are multiple factors to consider when creating a deep learning model. In training, accuracy on the test-set 
is the primary metric that is considered. In deployment, the performance of the model (latency/throughput) becomes 
a consideration for realtime applications and for reducing infrastructure costs in production.

Performance and accuracy, however, are at odds as models grow bigger and bigger.

### Sparsity: Improve Performance With Minimal Accuracy Loss
...
Importantly, Neural Magic uses *training-aware* pruning and quantization algorithms. Training-aware techniques 
apply the sparsification gradually, allowing the model to adjust by fine-tuning the remaining weights
with the training dataset at each step. This technique is critical to maintain high accuracy at high 
levels of sparsity.

<details>
    <summary>Click for conceptual details on pruning and quantization algorithms</summary>
...
2. **Unstructured Pruning** - any weight can be pruned in any pattern
...
matrix multiply faster. Additionally, techniques like quantization can be combined by unstructured pruning
...
**Post-Training** pruning and quantization optimizations are very easy to apply to a model. However, these techniques often create 
...
complex algorithms. However, by applying the pruning and quantization gradually and fine-tuning the non-zero weights 
...
For example, below is the sparsity profile of both an example CNN and Transformer:

|                               |ResNet-50  |BERT-base  |
|-------------------------------|-----------|-----------|
|% Pruned                       |95%        |95%        |
|Quantization                   |INT8       |INT8       |
|Accuracy % of Dense Baseline   |99%        |99%        |
|Throughput Improvment vs Dense |10x        |12x        |

You can see the full scope of pre-sparsified models in [SparseZoo](https://sparsezoo.neuralmagic.com/).
...
2. **Sparsification From Scratch**: apply training-aware pruning and quantization algorithms to any trained
    PyTorch, TensorFlow, and Hugging Face, with fine-grained control of hyperparameters

### Sparse Transfer Learning

Sparse Transfer Learning is the easiest path to creating a sparse model and is preferred for any scenario where a pre-sparsified 
foundation model exists in SparseZoo.
...
the sparsificaiton process) are publically available in the SparseZoo. 
...
- [Request a Model in Our Community Slack](https://discuss-neuralmagic.slack.com)

### Sparsification From Scratch

Sparsification From Scratch can be applied to any model, providing power-users a path to create sparse versions of models 
not yet supported in SparseZoo. 
...
**Global Magnitude Pruning (GMP)** is the best algorithm for unstructured pruning:
- With GMP, pruning occurs gradually over a training run. Over several epochs or training steps weights closest 
to zero are iteratively removed. The non-zero weights are then fine-tuned to the objective function. 
This iterative process enables the model to adjust to a new optimization pace after pathways are removed before pruning again. 

...
The first step is collecting the necessary configuration files. The text classification `Pipeline` is integrated with HuggingFace, and Therefore
uses HuggingFace Transformer's standards and configurations for model setup. The following files are required:
...
with SparseZoo, and supports supports SparseZoo stubs as inputs for automatic download and inclusion into easy testing and deployment.
...
# MARK: CLARIFY STEPS HERE

1. Create the `model.onnx` file (if you uses SparseML to train, the [ONNX export](https://github.com/neuralmagic/sparseml/tree/main/integrations/huggingface-transformers#exporting-to-onnx) creates the `model.onnx` file).
2. Collect the configuration files listed above. These are generally stored with the resulting model files from HuggingFace training pipelines.
3. Place the files into a directory and pass the local path to `model_path` in place of the SparseZoo stubs utilized below.
...
# MARK: DEMONSTRATE HOW TO PASS CUSTOM MODEL IN ADDITION TO ZOO STUB

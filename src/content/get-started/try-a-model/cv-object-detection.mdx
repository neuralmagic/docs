...
1. Create the `model.onnx` file (if you used sparseml to train, the [SparseML ONNX export](https://github.com/neuralmagic/sparseml/tree/main/integrations/ultralytics-yolov5#exporting-the-sparse-model-to-onnx) creates the `model.onnx` file).
2. Collect the config file and class names listed above.
3. Pass the local paths of the files in place of the SparseZoo stubs utilized throughout this document.

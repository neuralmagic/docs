...
With all of this built on top of DeepSparse's runtime, the simplicity of `Pipelines` is combined with GPU-class performance on CPUs for sparse models.
...
The `Pipeline` automatically downloads the necessary files for the model from the SparseZoo and compiles them on your local machine.

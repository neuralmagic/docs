...
The DeepSparse Platform enables you to (1) Optimize a Model for Inference and (2) Deploy Performantly and Easily on CPUs.

This tour orients you to our offering by showing you the major pieces of functionality and providing
...
By leveraging sparsification techniques like pruning and quantization, users can improve
performance when deploying on a sparsity-aware inference runtime like DeepSparse.

There are two workflows that allow users to accomplish this goal:
1. **[Sparse Transfer Learning (STL)](/get-started/transfer-a-sparsified-model)**: fine-tune pre-sparsified models 
from SparseZoo onto custom data while maintaining sparsity (RECOMMENDED)
2. **[Sparsification From Scratch (SFS)](/get-started/sparsify-a-model)**: apply pruning and quantization to any model
...
hyperparameters to reach high levels of sparsity.
...
to HuggingFace format while the YOLOv5 scripts adhere to Ultralytics format), but they generally look something like 
the following:
...
data must be provided in the HuggingFace format. 
...
instructs SparseML to maintain sparsity as the fine-tuning occurs. When Sparse Transfer Learning, you will almost always want to use a pre-made 
transfer learning recipe from SparseZoo passed as a SparseZoo stub. When Sparsifying From Scratch on a new model, you will likely create a 
custom recipe and run experiments with various hyperparameter settings.
...
Sparsification from Scratch will require you to create a recipe and experiement with hyperparameters, 
but the declarative nature of recipes makes makes it easy to focus on running experiments rather than writing code.

#### Additional Resources
- Reach out in our [Community Slack](https://app.slack.com/client/T020WGRLR8A/C020FPF3MQX) to request a new use case.
- Checkout our list of supported use cases
- Refer to [Use Cases](/use-cases) for details on the specific command line arguments for each.
- Refer to [Recipe User Guide](/user-guide/recipes) for details on recipes.

### Python API

For users needing flexibility for unsupported use cases or a supported use case
with custom training pipelines or infrastructure, SparseML also provides Python APIs that enable you to 
integrate SparseML into a PyTorch or TensorFlow pipeline.

As such, users can apply Sparse Transfer Learning and Sparsification From Scratch with just three additional
lines of code around a training pipeline.
...
a YAML file called a Recipe. We discussed recipes above, but as a reminder, they encode the hyperparameters 
of the Sparse Transfer Learning or Sparsification From Scratch workflows.  
- `manager.modify(...)` edits the `model` and `optimizer` objects to run the Sparse Transfer Learning or 
Sparsification From Scratch algorithms with the hyperparameters encoded in the recipe.
...
#### Additional Resources
- Reach out in our [Community Slack](https://app.slack.com/client/T020WGRLR8A/C020FPF3MQX) if you need help.
- Refer to [Custom Integrations Guide](/get-started/sparsify-a-model/custom-integrations) for more details on the Python API.
- Refer to [Recipe User Guide](/user-guide/recipes) for details on creating recipes.

If interested, checkout our [conceptual guide on optimizing for inference with sparsity](/index/optimize-workflow).
...
We have already explained the benefits of CPU-only deployments with DeepSparse in our [conceptual guide on deploying on CPUs](/index/deploy-workflow),
so this page will walk through how to integrate DeepSparse into an application.
...
Interested in another language? Reachout in [Community Slack](https://app.slack.com/client/T020WGRLR8A/C020FPF3MQX).
...
meaning you can pass raw strings to the Pipeline and a DeepSparse Pipeline in the Object Detection domain handles 
input normalization (mean and std transform) as well as the non-max supression of output, meaning you can just pass 
raw images and recieve the bounding boxes. 
...
For an unsupported use case, you will use `CustomTaskPipeline` to create a Pipeline. Simplify specify a 
...
#### Additional Resources
- [Pipeline Example (Supported Use Case)](/get-started/use-a-model) - introductory code examples using Pipelines
- [Pipeline Example (Custom Use Case)](/get-started/use-a-model/custom-use-case) - introductory code creating custom Pipelines
- [Use Cases](/use-cases/natural-language-processing/question-answering) - detailed examples of each supported Use Case
...
Multi-Stream Inference, and Dynamic Batch. You can checkout these utilities in the User Guide:
- Data Logging [Docs Coming Soon]
...
endpoints:
    - task: [task_name]     # specifiy use case (e.g. image_classification, question_answering)
      ...
      model: [mode_path]    # specify sparsezoo stub or path to local onnx file
...
#### Additional Resources
- [Server Example](/get-started/deploy-a-model) - introductory code examples using Server
- [Use Cases](/use-cases/natural-language-processing/question-answering) - detailed examples of each supported Use Case
- List of Supported Use Cases [Docs Coming Soon]
...
You can checkout these utilities in the User Guide:
- Server Documentation 
- Data Logging [Docs Coming Soon]
- [Multi-Stream Scheduling Overview](/user-guide/deepsparse-engine/scheduler)
- Example Using Multi-Stream in Server [Docs Coming Soon]
- Data Logging [Docs Coming Soon]

### Engine

Engine is the lowest supported level of interaction available with the runtime (Pipeline wraps Engine with utilities).
...
The `Engine` class is the workhorse for this pathway. The simply call the constructor with your desired parameters to 
create an instance with the runtime. Once initialized, just a pass lists of numpy arrays (which are a batch of input tensors - the 
same as would be passed to a PyTorch model) and the Engine will return a list of outputs.
...
in [Community Slack](https://app.slack.com/client/T020WGRLR8A/C020FPF3MQX)for details of this.
...
In addition to supported integrations, SparseML is set up as a framework to enable easy inclusion into custom pipelines to create a custom integration.
This flexibility enables easy sparsification for any neural network architecture for custom models and use cases.
Once SparseML is installed, the necessary code is importable through Python and can be plugged into most pipelines with only a few lines of code and recipes.
...
To do this properly in PyTorch, the ScheduledModifierManager requires the instance of the model to modify, the optimizer used for training, and the number of steps per epoch to ensure algorithms are applied at the right time.
...
To dive into the details of this recipe and how to edit it, visit that doc.

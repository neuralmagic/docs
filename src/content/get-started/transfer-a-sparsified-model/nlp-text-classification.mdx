...
1. Fine-tune a dense teacher model from SparseZoo (BERT) onto a new dataset (SST-2)
2. Transfer learn a pre-sparsified model (DistilBERT) from the SparseZoo onto SST-2, distilling from the dense teacher model trained in step 1
...
The `sparseml.transformers.text_classification` command installed with SparseML is used to fine-tune the dense teacher onto the SST-2 dataset for sentiment analysis.
...
# MARK: EXPLAIN HOW TO UTILIZE CUSTOM DATASET
# MARK: EXPLAIN WHAT THE KEY ARGUMENTS ARE / OPTIONS FOR THEM (model name recipe, task_name)
...
SparseZoo contains several pre-sparsified NLP models and recipes ready to be used as the student. This example uses the SparseZoo stub is DistilBERT:
```bash
...
After the command completes, the trained sparse model will be around 90.5% accurate on SST-2 and stored in the local `models/sparsified` directory.
...
To utilize your own custom dataset, pass in the appropriate text files for the `--train_file` and `--validation_file` arguments.

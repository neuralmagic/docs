...
This page will walk through an example of deploying a text-classification model with DeepSparse Server.
...
See an example of how to setup `Pipelines` in the [Test a Model](../../test-a-model) section.

Once the `Pipelines` are setup, the `deepsparse.server` command launches a server with the model at `--model_path` inside. The `model_path` can either

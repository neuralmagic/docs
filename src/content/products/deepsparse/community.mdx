...
DeepSparse's key feature is its performance on commodity CPUs, especially with inference-optimized sparse models.

For unoptimized dense models, DeepSparse's performance is competitive with other CPU-inference runtimes like ONNX Runtime. 
However, when optimization techniques like pruning and quantization are applied to a model, DeepSparse achieves an
order-of-magnitude performance gain over CPU-alternatives.

### Benchmarking

To demonstrate DeepSparse's performance gains from sparsity, let's compare DeepSparse and ONNX Runtime's performance on 
BERT with DeepSparse's Benchmarking utility. 

In SparseZoo, there is a 90% pruned-quantized version of BERT (i.e. 90% of the weights are removed and the weights are 
quantized to INT8), which retains >99% of the accuracy of the baseline dense model. 

Running this model on a 32 core `c6i.16xlarge` AWS instance, **DeepSparse achieves a 12.5x performance gain over ONNX Runtime**!
...
DeepSparse achieves 226 items/second with the 90% pruned-quantized version of BERT model.
```
...
**Pro-Tip:** In this example, we passed [SparseZoo](https://sparsezoo.neuralmagic.com/) stubs to the benchmarking script, 
which identify an ONNX file in Neural Magic's open-source repository of sparse models. You can also pass a

...
- `--recipe_path` instructs SparseML to run the sparsification process during the training loop. It can be either the stub of a recipe in the SparseZoo or a path to a local custom recipe. See [Creating Sparsification Recipes](/user-guide/recipes/creating) for more information.

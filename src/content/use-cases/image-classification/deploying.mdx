...
This integration enables you to easily deploy sparsified image classification models with DeepSparse for GPU-class performance directly on the CPU.
...
To deploy an image classification model with DeepSparse, pass the model in the ONNX format.
This grants DeepSparse the flexibility to serve any model in a framework-agnostic environment.
...
Refer also to the [full documentation for DeepSparse Server](/user-guides/deploying-deepsparse/deepsparse-server).

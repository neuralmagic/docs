...
rn50_embedding_pipeline = Pipeline.create(
    ...
    emb_extraction_layer=-3, # extracts last layer before projection head
)

input_image = "goldfish.jpg" # path to input image
...
In the NLP domain, you can use `transformer_embedding_extraction` as the `task` for some extra utilities:

The first is automatic embedding layer detection. If you use `emb_extraction_layer=1` (the default),
...
The second is usage of the `extraction_strategy` to perform a reduction on the sequence dimension rather 
than returning an embedding for each token. The options are:
...
bert_emb_pipeline = Pipeline.create(
    ...
    model_path="zoo:nlp/masked_language_modeling/bert-base/pytorch/huggingface/wikipedia_bookcorpus/pruned80_quant-none-vnni",
#     emb_extraction_layer=-1,         # (default: detect last layer)
    extraction_strategy="reduce_mean"  # (default: concat embedding for each token)
)

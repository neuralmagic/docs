...
This creates `model.onnx` file and exports it to **XXXXXXX** (e.g. `/trained_model/model.onnx`). `tokenizer.json` and `config.json` are also stored in this directory.
All of the examples below use SparseZoo stubs, but you can pass **XXXXXXX** to use the local custom model.

# MARK: CONFIRM ABOVE IS TRUE AND FILL IN XXXXX

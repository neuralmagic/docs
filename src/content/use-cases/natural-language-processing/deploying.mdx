...
- `tokenizer.json` - The [Hugging Face tokenizer file](https://huggingface.co/docs/transformers/fast_tokenizers)
- `config.json` - The [Hugging Face configuration file](https://huggingface.co/docs/transformers/main_classes/configuration)
...
### HTTP Server
As an alternative to Python API, DeepSparse Server allows you to serve ONNX models and pipelines in HTTP.
...
For full documentation on deploying sparse transformer models with DeepSparse Server, see the
[documentation](/user-guide/deploying-deepsparse/deepsparse-server).

...
The Hugging Face integration enables you to easily deploy sparsified Transformers onto the DeepSparse Engine for GPU-class performance directly on the CPU.
...
### Model Format
To deploy a Transformer using DeepSparse Engine, pass the model in the ONNX format along with the Hugging Face supporting files.
...
- `tokenizer.json`&mdash;the [HuggingFace tokenizer file](https://huggingface.co/docs/transformers/fast_tokenizers)
- `config.json`&mdash;the [HuggingFace configuration file](https://huggingface.co/docs/transformers/main_classes/configuration)
...
### HTTP Server
As an alternative to the Python API, the DeepSparse Server allows you to serve ONNX models and pipelines in HTTP.
...
Refer to the [full documentation](/use-cases/deploying-deepsparse/deepsparse-server) on deploying sparse transformer models with the DeepSparse Server.
...
For Neural Magic Support, sign up or log into our [Deep Sparse Community Slack](https://join.slack.com/t/discuss-neuralmagic/shared_invite/zt-q1a1cnvo-YBoICSIw3L1dmQpjBeDurQ). Bugs, feature requests, or additional questions can also be posted to our [GitHub Issue Queue](https://github.com/neuralmagic/sparseml/issues).

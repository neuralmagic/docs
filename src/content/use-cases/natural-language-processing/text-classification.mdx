...
The most important arguments are `model_name_or_path`, `task_name`, and `recipe`:
- `--model_name_or_path` indicates which model to start the pruning process from. It can be a model name, SparseZoo stub, or a path to a local model.
- `--recipe` argument instructs SparseML to run the sparsification process during the training loop. It can either be the stub of a recipe in the SparseZoo or a path to a local custom recipe. For more on creating a recipe see [here](/user-guide/recipes/creating).
- `--task_name` indicates that we should fine tune on the MNLI dataset.

# MARK: EXPLAIN HOW TO PASS CUSTOM DATASET OR MODEL
...
- `--model_name_or_path` indicates which model to start the pruning process from. It can be a model name, SparseZoo stub, or a path to a local model.
- `--recipe` argument instructs SparseML to run the sparsification process during the training loop. It can either be the stub of a recipe in the SparseZoo or a path to a local custom recipe. For more on creating a recipe see [here](/user-guide/recipes/creating).
- `--task_name` indicates that we should fine tune on the SST2 dataset.

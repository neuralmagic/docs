...
- (1) Optimize a Model for Inference with SparseML [(Follow Along In Google Colab)](https://colab.research.google.com/drive/13VSuy5PoH8wPZXuhowX4TPUoFCHIGBHQ#scrollTo=YNDMQPNrt836)
...
quantization (which together we call "sparsity").
...
Generally, you can use the Sparse Transfer Learning recipes from SparseZoo off the shelf, just 
adjusting the number of epochs depending on your dataset size.

We have a [full guide on recipes](user-guide/recipes/creating), but to give some intuition, 
a Sparse Transfer Learning recipe use two important modifiers:
...
With the command below, a dense version of **[XXX]** is sparsified using **[XXX]** dataset, using 
a pre-made sparsification recipe in SparseZoo.
...
We used a dense model from the SparseZoo and a pre-made recipe from the SparseZoo
as an example. However, if you are Sparsifying from Scratch you may want to use a model we do not 
have yet with a custom use case. To use the CLI script with a custom model/recipe, first export 
your model to the `.pt` format and create a `recipe.yaml` file (the example we have above should work 
for any model) in your local directory.
...
For this example, we will apply the simple custom recipe from above to **[XXX]** model, which is not in SparseZoo.

First, create a YAML file with the simple recipe from above and name it `your_recipe.yaml`. You will need to update line **[XXX]** in the 
...
DeepSparse enables you to deploy your models on CPUs, with GPU-class performance on sparse models. 
...
Pipelines have many additional features and settings you can configure the runtime's performance. 
...
The code below provides an examples benchmarking a dense and sparse version of [XXX] model on DeepSparse with an 
[XXX] core machine (AWS [XXX] instance):

#### Dense (Unoptimized) [XXX] 

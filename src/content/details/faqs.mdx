...
Neural Magic was founded by a team of award-winning MIT computer scientists and is funded by Amdocs, Andreessen Horowitz, Comcast Ventures, NEA, Pillar VC, and
Ridgeline Partners. It has several components, including [DeepSparse,](/products/deepsparse) a CPU runtime that runs sparse models at GPU speeds. To enable companies the ability to use
...
DeepSparse, created by Neural Magic, is a general purpose engine for machine learning, enabling machine learning to be practically
...
CPUs. DeepSparse achieves its performance using breakthrough algorithms that reduce the computation needed for neural network execution
...
from quantization will be greater. The  DeepSparse has INT8 support for the ONNX operators QLinearConv, QuantizeLinear,
...
DeepSparse optimizes _how_ the model is run on the infrastructure resources applied to it. But, the Neural

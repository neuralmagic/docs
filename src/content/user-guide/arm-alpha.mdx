...
In our recent 1.4 product release, we launched alpha support for DeepSparse on AWS Graviton and Ampere. We are working toward a general release across ARM server, embedded (like Raspberry Pi), and mobile (like Android) platforms in the first half of 2023.
...
- Setting up the DeepSparse ARM Alpha on an AWS Graviton or Ampere system
...
With sparsity and quantization applied to the model, DeepSparse achieves an additional performance speedup. With an 80% 4-block pruned-quantized version of BERT, DeepSparse achieves 71 items/second - **this is a 4.3x performance gain over the ORT baseline**!

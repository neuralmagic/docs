...
As we approach general release, we plan to push sparse performance signficiantly further with continued optimizations, including support for speedups
from unstructured sparsity combined with quantization. For instance, on a 16 core x86 AVX512 machine, DeepSparse runs a 90% pruned-INT8 quantized BERT model runs 2x faster than a 90% pruned-FP32 BERT model. These same optimizations apply in the ARM world, meaning we will push performance signficantly further than our already promising results at GA.

...
[ONNX](https://onnx.ai/) is a generic representation for neural network graphs to which most ML frameworks can be converted.
Some inference engines such as DeepSparse natively take in ONNX for deployment pipelines, so convenience functions for conversion and export are provided for the supported frameworks.

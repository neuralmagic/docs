...
Given the architecture above, to run DeepSparse on the first four CPUs on the second socket, you would use the following:
...
`NM_BIND_THREADS_TO_CORES` should be used with care since it forces DeepSparse to run on only the threads it has been allocated at startup. If any other process ends up running on the same threads, it could result in a major degradation of performance.

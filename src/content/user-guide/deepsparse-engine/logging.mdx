...
- **System Logging** is *enabled* by default. If not `logger` is specified, a `PythonLogger` logs to `stdout`.
- **Data Logging** is *disabled* by default. The config allows you to specify what data to log:

The YAML configured as follows.

`loggers` is a dictionary of the form `{logger_name: {arg_1: arg_value_1}}` and identifies and provides the arguments for each logger.

`data_logging` is a dictionary of the form `{identifier: [log_config]}` and identifies which data should be logged to where and how often. 

`identifier` specifies at which pipeline `stage` the logging in the list of `log_config` should occur. There are three forms `identifier` could take:
- `stage` - (one of `pipeline_inputs`, `engine_inputs`, `engine_outputs`, `pipeline_outputs`). Applies the each function in the `log_config` to the data type at the `stage`
- `stage.property` - If the data type at a `stage` has a property, you can unpack before applying the functions in the `log_config` like so.
- `stage.property[idx]` - If the `property` is a dict or a list, you can access via slicing, indexing, or dict access, for example `stage.property[0:2][0, :-1]['key_3'][5]`.

`[log_config]` is list of function configurations of the form `{func: name, frequency: freq, target_loggers: [logger_names]}`. 
...
Here's a tangible example for an image classification endpoint.
...
- System logging is enabled by default and logs to Prometheus and StandardOuput
...
loggers:
  prometheus:
    port: 6100  # < optional, if not provided defaults to 6100
```

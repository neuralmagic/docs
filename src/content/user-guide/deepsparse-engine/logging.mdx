...
DeepSparse Logging gives you telemetry to monitor a deployment. 

There are many types of monitoring tasks that you may want to perform with the data you log. The difficulty of the tasks varies from relatively easy 
...
System Logging gives you access to granular performance metrics for quick and efficient diagnosis of deployment system health.
...
4. `pipeline_outputs`: final output returned to the pipeline caller
...
2. Framework Functions - functions from `torch` or `numpy`
3. Custom Functions - custom user-provided functions

## Configuration

The Server YAML file is used to configure both System and Data Logging.
...
The YAML is configured as follows:

`loggers` configures the loggers. It is a dict of the form `{logger_name: {arg_1: arg_value}}`. 
...
`identifier` specifies the stages where logging should occur. It can either be a `stage` or `stage.property` if the data type
...
`log_config` specifies which the function to apply, which logger(s) to use, and how often to log. It is a dict of the form `{func: name, frequency: freq, target_loggers: [logger_names]}`. 
    ...
    data_logging:
      ...
      stage:
        ...
        - func: path/to/custom.py:bar  
          frequency: 10000
          # target_loggers: < if not specified, deaults to all

      # ... or a slice/dict access to a stage ...
      stage[slice]:
        - func: path/to/custom.py:bar  
          frequency: 1000
      ...
      # ... or a slice/dict access to a property
      stage.property[slice]:
        - func: np.fn_name        
          frequency: 1000          
...
endpoints:
  - task: image_classification
    ...
    data_logging:
      ...
      engine_inputs[0][0,:,:,0] # [np.array(BxHxWxC)]
        - func: np.mean
          frequency: 1
          target_loggers:
            - prometheus
...
- Logs the mean pixel of the first channel of the first engine input to Prometheus/StdOut
...
DeepSparse is integrated with Prometheus. The Prometheus Logger accepts some optional arguments as follows:

```yaml
loggers:
  prometheus:
    port: 6100
    text_log_save_frequency: 10,
    text_log_save_dir: /path/to/text/log/save/dir
    text_log_file_name: text_log_file_name
```
...
and implements the `log` method. The `log` method is called at each Pipeline stage (`pipeline_inputs`, 
`engine_inputs`, `engine_outputs`, `pipline_outputs`) and should handle exposing the metric to the Logger. 
...
## Tutorials
There are examples integrating Prometheus and DeepSparse logging available on GitHub:

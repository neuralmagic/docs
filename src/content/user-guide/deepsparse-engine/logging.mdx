...
`loggers` is a dictionary of the form `{logger_name: {arg_1: arg_value_1}}` and configures the loggers.

`data_logging` is a dictionary of the form `{identifier: [log_config]}` and identifies what and how data should be logged.
...
a numeric value like an Int or a Float.

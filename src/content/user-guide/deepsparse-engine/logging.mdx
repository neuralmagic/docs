...
the input data distribution post-facto). Examples include:
...
There is one group of System Logging Metrics currently available: Prediction Latency.  For each inference request, DeepSparse logs the following:
...
2. `engine_inputs`: pre-processed tensors passed to the engine for the forward passed

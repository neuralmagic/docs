...
Launch the server with the following (make sure [you have installed DeepSparse Server](/get-started/install/deepsparse)):

```bash
deepsparse.server --config-file server-config.yaml
```

Submit a request to the image classification endpoint with the following:
...
files = [("request", open(img, 'rb')) for img in paths]
resp = requests.post(url=url, json=files)
print(resp.text)
```

You should see the metrics logged to the Server's standard output and to Prometheus (look at `http://0.0.0.0:6100` to quickly see which metrics are exposed).